...
| `/chat` | Memory-enhanced chat and agent mode with provider/model selection, visible model-used metadata, context usage meter, stop-generation control, app-intercepted read-only wiki tool calls, compact local history, text/image attachments, Ctrl+V pasted images, retained drafts, and clickable resources. |
...
| `Chat:*` | Chat provider, Ollama endpoint/model/context-window metadata, GitHub fallback model order, prompt path, context/history/attachment byte/text limits, app-intercepted wiki tool-call limits, and agent write setting. |

The effective redacted configuration is visible at `/health` and `GET /api/diagnostics`.

User-created markdown pages under `Data/Pages` are part of the project wiki and should be committed unless explicitly temporary or private.
