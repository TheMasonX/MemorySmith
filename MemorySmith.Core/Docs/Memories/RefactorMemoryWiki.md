...
- 2026-05-12: Added hybrid search as a third search mode. The implementation uses Lucene.NET StandardAnalyzer tokenization for lexical ranking, reuses the local semantic scorer as the current semantic/vector-side signal, fuses rankings with RRF, and exposes the path through `/api/memories/search/hybrid`, `/memories` Hybrid mode, and `memorysmith_hybrid_search`.
- 2026-05-12: Added `memorysmith_context_pack` for larger KB agent workflows. The tool starts from hybrid search roots and packages linked references/conflicts with relationship metadata, scores, match reasons, tags, and bounded content so agents can gather useful project context without manual search/get chaining.
- 2026-05-12: Refined `memorysmith_context_pack` to accept explicit comma-separated root ids in addition to search queries. This makes context gathering more accurate after an agent already has candidate memory ids, while still allowing hybrid search discovery and reference/conflict expansion.
- 2026-05-12: Refined `memorysmith_context_pack` again for larger-KB accuracy: packs now report warnings for missing/invalid explicit ids and missing linked records, and can include backlinks through `includeBacklinks` so agents see records that point at the selected roots.
- 2026-05-12: Added `maxRecords` to `memorysmith_context_pack` so larger-KB packs have an explicit total record budget in addition to per-record content truncation. The service and MCP output warn when additional roots, references, conflicts, or backlinks are omitted because the budget was reached.
- 2026-05-12: Added `format=json` to `memorysmith_context_pack` while keeping Markdown as the default. Agents can now request the same bounded context pack as structured JSON with `query`, `generatedAt`, `records`, and `warnings` fields instead of parsing headings and labels.
- 2026-05-12: Added a purpose-built `project-wiki-test-fixture-*` memory subgraph to `Data/Memories/Core` for deterministic search, reference, conflict, backlink, `maxRecords`, and `format=json` testing. Updated `Schemas/memory.schema.json` to match the current PascalCase file-backed JSON shape and numeric `MemoryStatus` values while still allowing status names.
- 2026-05-17: Added optional ONNX Runtime semantic embeddings for memory search. `SemanticEmbeddingSearchService` performs exact in-memory cosine ranking when `SemanticSearch` model and WordPiece vocabulary assets are available, while `MemoryApplicationService` falls back to the existing token/tag/title/reference/alias scorer when assets are missing or inference fails.
