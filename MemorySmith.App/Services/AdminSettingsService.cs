...
public sealed class AdminSettingsService
{
    ...
    [
        ...
        EditableSettingDescriptor.Boolean("MemorySmith:Chat:AgentWritesEnabled", "Agent writes enabled", "Chat", settings => settings.Chat.AgentWritesEnabled, "Allows Agent mode write proposals and Agent-only mutation tools to be applied when user role and approval flow permit it. Chat mode remains read-only and cannot create memories/pages/tasks."),
        EditableSettingDescriptor.String("MemorySmith:Chat:AgentWriteApprovalMode", "Agent write approval mode", "Chat", settings => settings.Chat.AgentWriteApprovalMode, 100, "Controls Agent write application. Use manual for proposal review before writes; use auto_accept only in trusted environments where Agent writes and mutation tools may apply directly."),
