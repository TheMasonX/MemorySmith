...
public sealed class AdminSettingsService
{
    ...
    [
        ...
        EditableSettingDescriptor.StringList("MemorySmith:Training:SyntheticDataPaths", "Synthetic corpus paths", "Training", settings => settings.Training.SyntheticDataPaths, "Explicit SFT corpus files, one per line, used for training runs. Leave empty only when transcript or starter examples are intentionally enabled."),
        EditableSettingDescriptor.Boolean("MemorySmith:Training:IncludeTranscriptExamples", "Include transcript corpus in runs", "Training", settings => settings.Training.IncludeTranscriptExamples, "When enabled, training runs import transcript content JSONL examples from the configured transcript directory."),
        EditableSettingDescriptor.Boolean("MemorySmith:Training:IncludeStarterExamples", "Include bundled starter samples", "Training", settings => settings.Training.IncludeStarterExamples, "When enabled, training runs append bundled starter SFT examples. Keep disabled for explicit-corpus experiments."),
