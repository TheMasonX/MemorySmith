...
public class BackgroundServiceTelemetryTracker
{
    ...
    public void RecordRunStart(string serviceName, string interval)
    {
        ...
        _telemetry.AddOrUpdate(
            serviceName,
            _ => new BackgroundServiceTelemetry { ServiceName = serviceName, Interval = interval, LastRunUtc = DateTime.UtcNow },
            (_, existing) =>
            {
    ...
    public void RecordRunSuccess(string serviceName, double durationMs)
    {
        ...
        _telemetry.AddOrUpdate(
            serviceName,
            _ => new BackgroundServiceTelemetry { ServiceName = serviceName, LastRunUtc = now, LastSuccessUtc = now, LastDurationMs = durationMs },
            (_, existing) =>
            {
    ...
    public void RecordRunFailure(string serviceName, double durationMs)
    {
        ...
        _telemetry.AddOrUpdate(
            serviceName,
            _ => new BackgroundServiceTelemetry { ServiceName = serviceName, LastRunUtc = now, LastFailureUtc = now, LastDurationMs = durationMs },
            (_, existing) =>
            {
                existing.LastRunUtc = now;
