...
`/chat` uses the `IChatProvider` and `IChatAgent` abstractions. The registered providers are `OllamaChatProvider`, which calls a local Ollama HTTP service, and `GitHubCopilotChatProvider`, which uses the GitHub Copilot SDK with GitHub CLI authentication or a configured token environment variable. `MemoryChatAgent` now uses intent-aware preloading: exact-reply/simple prompts and write-only Agent commands skip local wiki pre-context, while explicit MemorySmith/wiki/codebase prompts receive a small bounded hybrid memory plus page preload. When preloaded context is absent or not enough, the shared prompt lets the model request an app-intercepted, MCP-compatible read-only MemorySmith tool call by returning JSON such as `{"toolCalls":[{"name":"memorysmith_hybrid_search","arguments":{"query":"search text","limit":10}}]}`. The chat allowlist includes read-only memory, page, task, and code tools: `memorysmith_search`, `memorysmith_hybrid_search`, `memorysmith_context_pack`, `memorysmith_get`, `memorysmith_page_search`, `memorysmith_page_get`, `memorysmith_task_list`, `memorysmith_task_get`, `memorysmith_code_search`, and `memorysmith_code_search_status`. `memorysmith_code_search` now supports `forceRebuild=true` for an explicit cold rebuild, while `memorysmith_code_search_status` reports live indexing progress, warm-reuse counters, the currently indexed document, and the last completed build summary. A deterministic intent interceptor (`ChatIntentInterceptor`) also pre-runs an obvious tool call when the user message starts with phrases like "search the wiki for ...", "search the codebase for ...", "open page &lt;slug&gt;", "get memory &lt;id&gt;", "semantic/hybrid search ...", or "context pack ...", so reliable retrieval does not depend on the model emitting the JSON tool-call protocol correctly. Retrieved tool output and preloaded context are wrapped in an "Untrusted retrieved data" preamble before being added to the model context so any embedded instructions are treated as data, not commands.
...
`MemorySmith:SemanticSearch:PrewarmOnStartupEnabled` now defaults to `true`. The app starts a background semantic prewarm after startup so the first semantic query or code-search request does not absorb the full lazy ONNX tokenizer/session initialization cost. Disable it only when you explicitly want cold-start measurements or you need to defer all embedding work until first use.

For CUDA on Windows, installing cuDNN alone is not enough if the installer leaves the DLLs in a versioned NVIDIA folder that is not on `PATH`. On the current maintainer machine, the required `cudnn64_9.dll` landed under `C:\Program Files\NVIDIA\CUDNN\v9.22\bin\12.9\x64`, and ONNX Runtime stayed in CPU fallback mode until that directory was made visible to the process environment.

`memorysmith_code_search_status` now reports a `build.timings` object with provider initialization, file read, content hash, chunk preparation, embedding, database write, and removed-document cleanup timings plus embedding call counts. On the current repo dataset, the main CUDA cold-build bottleneck is still the synchronous per-chunk embedding loop rather than SQLite writes, so a healthy CUDA provider can still lose to CPU fallback until batching is introduced.

For an end-to-end runbook, including Windows service deployment, local override examples, and repo code-index warming/profiling, use [Semantic Acceleration Setup Guide](Data/Pages/guides/semantic-acceleration-setup.md).

## MCP Tools

The MCP endpoint is at `http://localhost:5089/mcp` for the default local HTTP launch profile, and the HTTPS/LAN path is `https://memorysmith.home.arpa:7090/mcp` when you follow the repo's private-network certificate setup. The checked-in VS Code workspace config in `.vscode/mcp.json` intentionally targets `https://memorysmith.home.arpa:7090/mcp` so the workspace uses the same private alias as the LAN HTTPS guidance. To make that work on another machine, keep the alias, make `memorysmith.home.arpa` resolve to the MemorySmith host, and trust a certificate whose SAN includes `memorysmith.home.arpa`; see `Data/Pages/guides/https-setup.md` for the concrete certificate and hosts-file steps. If you need a one-off local override, change `.vscode/mcp.json` in your working copy and keep that override uncommitted. Up to twenty-one tools are exposed by default (ten read-only chat-allowlisted tools, seven task tools, two page write tools requiring edit permission, plus two source-aware tools available only over MCP):
...
| `memorysmith_code_search` | `query`, `targets`, `limit`, `rebuildIfStale`, `forceRebuild` | Structured JSON result set with indexed file paths, line ranges, snippets, score, local absolute paths, and current index status | View |
| `memorysmith_code_search_status` | none | Structured JSON status with indexed counts plus live/last build progress, warm reuse counts, and the current document when a build is running | View |
...
- `memorysmith_search` and `memorysmith_hybrid_search` use the same plain-text query contract as the REST search endpoints.
- `memorysmith_code_search` searches the configured repo targets (`MemorySmith.App`, `MemorySmith.Core`, `MemorySmith.Storage`, `MemorySmith.Tests`, and `MemorySmith.Benchmarks` by default), respects the repo `.gitignore`, supports optional include or exclude overrides through configuration, stores its reusable chunk index in `Data/Graph/code-search/code-search.db`, batches document writes, reuses unchanged documents on warm builds, and accepts `forceRebuild=true` when the caller wants a cold rebuild from source.
- `memorysmith_code_search_status` exposes build visibility for the same index, including whether a build is running, how many files have been processed, reused, updated, skipped, removed, or failed, the current document path, and the last completed build state.
...
- `memorysmith_page_search` and `memorysmith_page_get` do not currently expose a `format` switch. Use `memorysmith_code_search` when you need structured results.
...
- `memorysmith_code_search` also requires the normal view policy; it reads only the configured repo targets and returns snippets plus file paths from the indexed code cache rather than raw shell or external-tool access.
- Page tools also respect each page's minimum visibility role, so `memorysmith_page_search` and `memorysmith_page_get` omit pages the caller cannot view.
...
```json
{
  "MemorySmith": {
    ...
    "TaskAttachments": {
      "StoragePath": "../artifacts/task-attachments",
    ...
    "Chat": {
      ...
      "AgentWriteApprovalMode": "manual",
      "AgentWriteRoots": [ "../Data/Memories/Working", "../Data/Pages" ]
    }
  }
}
```
...
- **`ApiKey`** — if set, guarded non-loopback API and MCP requests must include `X-Api-Key: &lt;value&gt;`. Leave `null` for loopback local use only. MemorySmith intentionally exempts the browser-facing auth and first-admin setup routes (`/api/auth/*` and `/api/admin/setup*`) so LAN UI sign-in can still work, but the shared API key remains required for guarded remote API/MCP access. The shared API key can satisfy non-admin API/MCP policies; it does not grant admin, user-management, settings, audit, diagnostics, or restore access.
- **`AllowRemoteApi`** — set `true` to allow non-localhost API/MCP callers after `ApiKey` is configured. Off by default.
- **`SecurityProfile`** — optional preset for related security defaults. Use `secure-local` for the recommended dogfood/local posture, `local-dev` for permissive local iteration, and `remote-hardened` for exposed instances. Leave `null` to keep the explicit settings as written.
- **`LocalDevelopment` environment behavior** — when `ASPNETCORE_ENVIRONMENT=LocalDevelopment`, `MemorySmithLocalDevelopmentPostConfigure` also applies additional dogfood-friendly defaults for missing keys such as remote API enablement, relaxed remote-auth HTTPS, raw HTML pages, larger chat/tool limits, and agent writes. Explicit overrides in `appsettings.LocalOverrides.json` or the configured `SettingsOverridePath` still win.
