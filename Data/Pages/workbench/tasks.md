...
| Done | Copilot | Tighten Pages desktop navigation footprint and tree density | `/pages` now uses a slimmer desktop navigation footprint with denser tree rows and lighter folder-count chrome so the selected page gets more horizontal reading space without changing Tree/Flat/ToC behavior. | Fresh-host browser check on `http://127.0.0.1:5126/pages` |
| Done | Copilot | Default empty chat sessions to sidebar-closed for main-pane space | `/chat` now opens empty sessions without the History/Trace rail visible by default, keeping the main transcript/composer width available until the user explicitly requests the sidebar. | Fresh-host browser check on `http://127.0.0.1:5126/chat` |
| Done | Copilot | Rebalance Pages navigation and reading space on narrow viewports | `/pages` now gives the selected reader more of the first viewport by shrinking the compact nav row, tightening rendered-page padding, and softening narrow code-block overflow while keeping Tree, Flat, ToC, and focus-reading controls reachable. | `artifacts/browser-validation/tsk-0130/20260526-140123-interactive/` |
| Done | Copilot | Compact the Memories workbench filter and detail layout | `/memories` now keeps only primary search in the first row, moves status/tags/search-mode controls behind a compact `Search options` expander, and folds references/source links into a secondary `Related context` panel so the selected memory body keeps the first viewport on narrow screens without a nested link scroller. | `artifacts/browser-validation/tsk-0124/20260526-135259-interactive/` |
