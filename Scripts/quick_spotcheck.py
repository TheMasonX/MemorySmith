#!/usr/bin/env python3
"""Quick A/B spotcheck runner — smaller case set for smoke validation."""
import argparse
import json
import re
...
def build_cases() -> list[EvalCase]:
    # Reuse the same case distribution as the full spotcheck but allow slicing.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Quick A/B tool-call spot check (smoke)")
    ...
    cases = build_cases()[: args.case_count]
    ...
    payload = {
        ...
        "tuned": {"summary": summarize(tuned_rows), "rows": tuned_rows},
        "caseCount": len(cases),
    }
    ...
    print(f"[{utc_now_iso()}] Wrote quick results to {output_path}")
    return 0
